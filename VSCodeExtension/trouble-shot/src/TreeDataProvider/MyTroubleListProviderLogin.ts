...
export class MyTroubleListProviderLogin extends MyTroubleListProvider {
  ...
  async getChildren(): Promise<Trouble[]> {
    ...
    const res = await fetch(
      `http://orientalsalad.kro.kr/api/troubleshooting/trouble-shootings?pageSize=10&writerSeq=${sessionId}&loginSeq=${sessionId}`,
      {
        method: "GET",
        headers: {
  ...
  async deleteTroubleShooting(trouble: Trouble) {
    const sessionId = this.globalState.get<number>("sessionId");
    const res = await fetch(
      `http://orientalsalad.kro.kr/api/troubleshooting/trouble-shootings/${
        trouble.id
